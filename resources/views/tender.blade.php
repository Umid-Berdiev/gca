                ...
                @section('nowosti')
                    ...
                    <div class="page-header row">
                        ...
                        <div class="col-md-3 hidden-xs hidden-sm" style="padding-top: 11px;">
                            ...
                            <a class="rss-link pull-right" href="{{URL(\Illuminate\Support\Facades\App::getLocale().'/rss/tender')}}"><img src="{{URL('/images/Feed-icon.svg.png')}}" alt="" width="20" height="20"> RSS</a>
                        </div>
                    </div>
                    <br>
                    <div class="col-sm-12">
                        ...
                        <div class="tenders-filter">
                            <form action="{{URL(App::getLocale().'/tenders/filter')}}" method="get" name="tenders_archive_filter">
                                ...
                                <table width="100%" class="text-center">
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="{{URL(App::getLocale().'/tender/'.$curcat->group)}}"  name="clear"  class="btn btn-warning">Тозалаш</a></td>
                                    </tr>
                                    </tbody>
                                </table>
                            </form>
                        ...
                        <div class="item">
                            <div class="row section-to-print">
                                <div class="col-md-3 col-sm-4">
                                    <br/><a href="#"><img class="img-responsive" src="{{ URL(App::getLocale().'/downloads?type=tenders&id='.$value->group) }}"></a>
                                </div>
                                <div class="col-md-9 col-sm-8">
                                    <h4><a href="{{ URL(App::getLocale().'/tender/'.$curcat->group.'/'.$value->group) }}">{{ $value->title }}</a></h4>
                                    <h5><span class="label label-primary"> {{ $value->created_at }} </span></h5>
                                    <p style="text-align: justify;">{!! $value->description  !!} </p>
                                    <div class="row">
                                        <div class="col-sm-5"><p><a href="#">Соҳа:</a> {{$value->category_name}}</p></div>
                                        <div class="col-sm-5"><p><a href="#">Чоп этиш санаси:</a>{{ $value->deadline }} </p></div>
                                    </div>
                                </div>
                            </div><!-- .row -->
                        </div><!-- .item -->
                        ...
                        <div class="list-group">
                            @foreach($newscat as $value)
                                @if ($curcat->group == $value->group)
                                    <a href="{{ URL(App::getLocale().'/tender/'.$value->group) }}" class="list-group-item active">{{$value->category_name}}</a>
                                @else
                                <a href="{{ URL(App::getLocale().'/tender/'.$value->group) }}" class="list-group-item">{{$value->category_name}}</a>
                                @endif
                                    @endforeach
