                ...
                @section('nowosti')
                    ...
                    <div class="page-header row">
                        ...
                        <div class="col-md-3 hidden-xs hidden-sm" style="padding-top: 11px;">
                            ...
                            <a class="rss-link pull-right" href="{{URL(\Illuminate\Support\Facades\App::getLocale().'/rss/post')}}"><img src="{{URL('/images/Feed-icon.svg.png')}}" alt="" width="20" height="20"> RSS</a>
                        </div>
                    </div>
                    <br>
                    @if ($errors->any())
                        ...
                        <form action="{{URL(App::getLocale().'/postf/filter')}}" method="get" name="tenders_archive_filter">
                            ...
                            <table width="100%" class="text-center">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="date" id="" name="start" value="@if(Request::has('start') && !empty(Request::get('start')) ){{ date("Y-m-d",strtotime(Request::get('start')))}}@endif" class="input-text form-control" autocomplete="off"><img src="#" alt="" class="" onclick="#" onmouseover="" onmouseout="" border="0"></td>
                                    <td> -- </td>
                                    <td>
                                        <input type="date" id="" name="finish" value="@if(Request::has('finish') && !empty(Request::get('finish'))){{date("Y-m-d",strtotime(Request::get('finish')))}}@endif" class="input-text form-control" autocomplete="off"><img src="" alt="" class="calendar-icon" onclick="" onmouseover="" onmouseout="" border="0"></td>
                                    <td> </td>
                                    <td><input type="submit" class="btn btn-warning" value="@lang('blog.filter_submit')"></td>
                                    <td><a href="{{URL(App::getLocale().'/posts/'.$curcat->group)}}" name="clear" value="@lang('blog.filter_reset')" class="btn btn-warning">@lang('blog.filter_reset')</a></td>
                                </tr>
                                </tbody>
                            </table>
                        </form>
                        ...
                        <div class="item " id="section-to-print">
                            <div class="row  ">
                                <div class="col-md-4 col-sm-4">
                                    <br><a href="{{ URL(App::getLocale().'/posts/'.$curcat->group.'/'.$value->group) }}"><img class="img-responsive" src="{{ URL(App::getLocale().'/downloads?type=post&id='.$value->group) }}"></a>
                                </div>
                                <div class="col-md-8 col-sm-8" >
                                    <h4 style="padding-top: 15px;"><a href="{{ URL(App::getLocale().'/posts/'.$curcat->group.'/'.$value->group) }}">{{ $value->title }}</a></h4>
                                    <?php $date=date_create($value->datetime) ?>
                                    <h4><span class="label label-primary"> {{date_format($date,"d.m.Y H:i")}} </span></h4>
                                    <p style="text-align: justify;"></p><p>{{ $value->decription }}...
                                    </p>
                                </div>
                            </div><!-- .row -->
                        </div><!-- .item -->
                    ...
                    <div class="col menu-item-structure">
                        ...
                        <div class="list-group">
                            @foreach($newscat as $value)
                                ...
                                <a href="{{ URL(App::getLocale().'/posts/'.$value->group) }}" class="list-group-item active">{{$value->category_name}}</a>
                                @else
                                    <a href="{{ URL(App::getLocale().'/posts/'.$value->group) }}" class="list-group-item">{{$value->category_name}}</a>
                                    @endif
                            @endforeach
